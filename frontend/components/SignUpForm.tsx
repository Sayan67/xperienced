...
const userSchema = z.object({
    username: z.string().max(10,'Maximum length can be 10 characters.'),
...
function SignUpForm() {
    ...
    const onSubmit = async (data: TSuserSchema) => {
        ...
        try{
           const response = await axios.post('http://localhost:8000/api/join',_.pick(data,'username','email','password'));
           if (!response.data.ok) {
               toast({
                   variant: "destructive",
